# app.R
# Versi 8.2: "Stable Custom Colors Edition"
# Oleh: Gemini (Geologist & Informatics Specialist)
#
# Changelog v8.2:
# - IMPLEMENTASI ULANG FITUR WARNA KUSTOM:
#   - Menambahkan kembali kemampuan untuk mengubah warna litologi secara manual.
#   - UI untuk pengaturan warna diintegrasikan ke dalam sidebar tab "Plot Downhole".
# - LOGIKA YANG STABIL:
#   - Menggunakan pendekatan reactive yang lebih sederhana dan andal untuk menangani
#     perubahan warna, mencegah crash yang terjadi pada versi sebelumnya.
# - KONSISTENSI VISUAL:
#   - Warna yang diatur pengguna kini diterapkan secara konsisten pada Plot Downhole
#     dan Boxplot.
#   - Palet warna default untuk plot lain ditingkatkan agar lebih jelas.
...
library(colourpicker) # Diperlukan untuk input warna
...
ui <- fluidPage(
  tags$div(
    ...
    div(style="padding-bottom: 50px;",
        navbarPage(
          "Analisis Data Bor Geologi v8.2 (Stable)",
          ...
          tabPanel("Plot Downhole",
                   sidebarLayout(
                     sidebarPanel(
                       ...
                       hr(),
                       # UI untuk pengaturan warna
                       h4("Pengaturan Warna Litologi"),
                       helpText("Ubah warna dan plot akan diperbarui secara otomatis."),
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(litho_colors = NULL)
  ...
  output$gradeHistogram <- renderPlotly({
    ...
    # Versi ini menggunakan `colors` yang bisa menyebabkan error di beberapa environment
  ...
  # --- 2e. Logika Pengaturan Warna Kustom (STABIL) ---
  ...
  observe({
    ...
    if (!identical(names(rv$litho_colors), unique_lithos)) {
      current_colors <- rv$litho_colors
      new_color_list <- list()
      
      palette <- colorRampPalette(brewer.pal(8, "Set2"))(length(unique_lithos))
      
      for(i in seq_along(unique_lithos)){
        litho_name <- unique_lithos[i]
        if(litho_name %in% names(current_colors)){
          new_color_list[[litho_name]] <- current_colors[[litho_name]]
        } else {
          new_color_list[[litho_name]] <- palette[i]
        }
      }
      rv$litho_colors <- new_color_list
    }
  })
  ...
  output$lithoColorUI <- renderUI({
    ...
    lapply(names(rv$litho_colors), function(name) {
  ...
  observe({
    ...
    for(name in names(rv$litho_colors)) {
      input_color <- input[[paste0("color_", name)]]
      if(!is.null(input_color) && temp_colors[[name]] != input_color) {
        temp_colors[[name]] <- input_color
      }
    }
    
    if(!identical(rv$litho_colors, temp_colors)){
